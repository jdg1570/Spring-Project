...
<body><br/>
<div class="container">
	<input type="button" value="←" onclick="location.href='${ctp}/member/Main';" class="btn btn-dark" style="margin-bottom: 20px;">
	<input type="button" value="내 서제에 담기" onclick="myBook()" class="btn btn-dark" style="float: right;">
	<table class="table">
		<tr>
			<td style="width: 15%;">
				${vo.bookContent}
			...
			<td>
				<table class="table-borderless" style="text-align: left;">
					...
					<tr>
						<td>
							<span style="color: gray; font-size: 14px;">이 책이 담긴 서재</span><br/>
							<a href="${ctp}/admin/AddList?bookIdx=${vo.bookIdx}"><span style="font-size: 15px; font-weight: bolder; color: black;">${mCnt}개</span></a>
						</td>
						<td>
							<span style="color: gray; font-size: 14px;">한 줄 리뷰</span><br/>
							<span style="font-size: 15px; font-weight: bolder;">${reviewCnt}개</span> 
						</td>
					</tr>
				</table>
			</td>
					...
					<c:forEach var="bvo" items="${bVos}">
						<p style="font-size: 13px; margin-left: 20px;">
							<c:if test="${bvo.nickName == sNickName}">
				              <a href="javascript:replyDelCheck(${bvo.idx});"><span class="glyphicon glyphicon-remove" style="float: right; padding-right: 10px;"></span></a>
				            </c:if>
							<i>${bvo.nickName}</i><br/>
							...
							<span style="font-size: 15px;">${fn:replace(bvo.content,newLine,'<br/>')}</span>
								<c:if test="${bvo.likeSw != 1}">
									...
									<i class="far fa-heart" id="Like" onclick="javascript:likeBtn(${bvo.idx})" style=" float:right; color: red; padding-right: 10px;"></i>
								</c:if>
								<c:if test="${bvo.likeSw == 1}">
									<span style="float: right; padding-right: 10px;">${bvo.likeSum}</span>
...
<script>
	...
	// 내 서재 담기
	function myBook() {
		var ans = confirm("내 서재에 담으시겠습니까?");
		if(!ans) {
			return false;
		}
		else {
			document.myform.action = "${ctp}/admin/ProductContent";

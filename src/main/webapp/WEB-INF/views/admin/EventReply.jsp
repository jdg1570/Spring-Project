...
<script>
	...
	function pageCheck() {
		...
		location.href = "${ctp}/admin/EventReply?page=${pag}&pageSize="+pageSize;
	}
	...
	function nickNameSearch() {
  		...
  		if(nickName == "") {
  			...
  			location.href="${ctp}/admin/EventReply?nickName="+nickName;
  		}
  	}
</script>
<style>
	  ...
	  <form name="myform" method="post">
		  <table class="table table-borderless m-0">
		    <tr class="text-center">
		      <td colspan="3"><h3>이벤트 댓글(총 : ${totRecCnt}개)</h3></td>
		    </tr>
		    <tr>
		      <td>
		      ...
		      <td>
		        <select name="pageSize" id="pageSize" onchange="pageCheck()" class="form-control" style="float: right; width: 120px; height: 30px; font-size: 13px;">
...
<div class="container">
		<ul class="pagination justify-content-center">
			...
			<c:if test="${totPage != 0}">
				  <c:if test="${pag != 1}">
				  	<li class="page-item"><a href="${ctp}/admin/EventReply?pag=1&pageSize=${pageSize}" title="첫페이지" class="page-link">첫페이지</a></li>
				  </c:if> 
				  <c:if test="${curBlock > 0}">
				 		<li class="page-item"><a href="${ctp}/admin/EventReply?pag=${(curBlock-1)*blockSize + 1}&pageSize=${pageSize}&nickName=${nickName}" title="이전블록" class="page-link">◀</a></li>
				  </c:if>
				  <c:forEach var="i" begin="${(curBlock*blockSize)+1}" end="${(curBlock*blockSize)+blockSize}">
				    <c:if test="${i == pag && i <= totPage}">
				   	 <li class="page-item active"><a href='${ctp}/admin/EventReply?pag=${i}&pageSize=${pageSize}&nickName=${nickName}'class="page-link text-light"><b>${i}</b></a></li>
				    </c:if>
				    <c:if test="${i != pag && i <= totPage}">
				    	 <li class="page-item"><a href='${ctp}/admin/EventReply?pag=${i}&pageSize=${pageSize}&nickName=${nickName}' class="page-link">${i}</a></li>
				    </c:if>
				  </c:forEach>
				  <c:if test="${curBlock < lastBlock}">
				  	<li class="page-item"><a href="${ctp}/admin/EventReply?pag=${(curBlock+1)*blockSize + 1}&pageSize=${pageSize}&nickName=${nickName}" title="다음블록" class="page-link">▶</a></li>
				  </c:if> 
				  <c:if test="${pag != totPage}">
				  	<li class="page-item"><a href="${ctp}/admin/EventReply?pag=${totPage}&pageSize=${pageSize}&nickName=${nickName}" title="마지막페이지" class="page-link">마지막페이지</a></li>
				  </c:if>
			</c:if>
		</ul>
	</div>
